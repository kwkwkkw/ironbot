...
async def who(event):
    ...
    await event.edit(
        "`Tunggu Master, berikan saya waktu untuk mencari info tentang orang ini`")
...
async def fetch_info(replied_user, event):
    ...
    caption += f"👤 Nama: {first_name} {last_name}\n"
    caption += f"📍 Username: {username}\n"
    caption += f"📂 Data Center ID: {dc_id}\n"
    caption += f"🌆 Jumlah Foto: {replied_user_profile_photos_count}\n"
    caption += f"🤖 Bot Check: {is_bot}\n"
    caption += f"⚡ Akun Bermasalah: {restricted}\n"
    caption += f"✔️ Verified by Telegram: {verified}\n"
    caption += f"🆔 ID: <code>{user_id}</code>\n\n"
    caption += f"💡 Bio: \n   <code>{user_bio}</code>\n\n"
    caption += f"👲 Chats common: {common_chat}\n"
    caption += f"👁️‍🗨️ Link ke Profile: "
